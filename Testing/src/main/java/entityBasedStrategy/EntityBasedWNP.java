...
//		dfFiltered.show(false);
		...
		Dataset<Row> dfPruned = wnpPruning(dfPreprocessing, dfTest, Integer.parseInt(args[0]));
...
//		df.show(false);
	...
	private static Dataset<Row> wnpPruning(Dataset<Row> df, Dataset<Row> dfTest, int scheme) {
		//int scheme = 2;
		if (scheme == 1) {
			//df.show(false);
