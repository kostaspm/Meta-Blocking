...
public class ComparisonBasedCNP {
...
//				ARCSScheme();
		...
		df = df.withColumn("Node2_Weight_Filtered", slice(df.col("Node2_Weight"), 1, index)).drop("Node2_Weight"); // third
																													// argument
																													// is
																													// the
																													// N
		...
		df = df.withColumn("Node", array(df.col("Node"), df.col("Node2_Weight_Filtered.col1")))
				.withColumn("Weight", df.col("Node2_Weight_Filtered.Weight")).drop("Node2_Weight_Filtered");
		df = df.withColumn("Node", array_sort(df.col("Node")));
		df = df.dropDuplicates();
